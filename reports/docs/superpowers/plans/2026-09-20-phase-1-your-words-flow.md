...
## Rulings after the live gate and the final review (the integrator's, 2026-09-20)

- **The article rework system prompt carries the reporting-mode block** (eighth prompt, 1f6cafb); the outline prompt's `<TEMPORAL_DISCIPLINE>` block and the core arc system prompt teach the timeline in the third person (230a86a, df43083). Render-diff expectations gain: `outline-generation.txt` two lines in the temporal block; `article-revision.txt` the mode block after the identity line and the `REVISION CONTEXT: ARTICLE (automated pass N)` header; `outline-revision.txt` the `REVISION CONTEXT: OUTLINE (automated pass N)` header. The render harness passes the session's mode to the article rework prompt.
- **An approval note survives a rollback** (ac0f8c6): `pruneGateNotes` drops rejection notes of the invalidated stops and keeps approval notes, because an approval note is forward guidance for the writer the rollback creates. So "With Approve it stands for every later writer" is true at the article stop too.
