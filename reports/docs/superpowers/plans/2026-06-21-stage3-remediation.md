...
## Phase P9 (corrected scope, added 2026-06-22 post-preflight)

The 2026-06-22 preflight (read of every P9 anchor against live source + a repo-wide sweep for tests/consumers asserting the OLD shapes) found the three tasks' *primary* edits are correct and their anchors all still resolve (with one drift), but **two downstream consumers were missed** by the per-phase author. Landing P9 as originally written would (a) fail an existing test under `__tests__/unit/` the author didn't see, and (b) break ~10+ tests and ship a console editor whose pull-quotes widget produces server-rejected outlines. Decision (user-approved 2026-06-22): **fold the full reconciliation into the existing tasks** (no new commits split a red suite). The original 9.1/9.2/9.3 bodies above remain authoritative for the primary edits; this block ADDS to them.

**Anchor drift (re-anchor by symbol, not line — P8 edited these files):**
- 9.1 `quote` variant: still `content-bundle.schema.json:123-132` (exact; `required` at `:125`).
- 9.2 prompt skeleton: still `prompt-builder.js:541-551` (exact). 9.2 outline schema: still `outline.schema.json:199-223` (exact); `thePlayers.required=["arcConnections"]`, `additionalProperties:false`, `characterHighlights` follows.
- 9.3 post-gen guards: **NOW `ai-nodes.js:1192-1212`** (plan body says `:1213-1233` — ~21 lines high; the `before` text matches byte-for-byte, anchor on `const minPullQuotes = themeDisplay.minPullQuotes ?? 2;`). `theme-config.js` `minPullQuotes:0` at `:91`/`:151` (unchanged).
- `SchemaValidator`: `new SchemaValidator()` → `.validate(name,data)` → `{valid, errors:null-when-valid}`; all schemas compile `strict:true` at construction (a malformed outline edit throws there).

**Gap A — fold into Task 9.1 (CR-2):** an existing test asserts the OLD required-attribution shape and will go red when 9.1 relaxes it. The plan body's Step 5 names `lib/__tests__/schema-validator.test.js` ("if it does not exist…") — the real file is `__tests__/unit/schema-validator.test.js`, which the author never saw.
- Add a step in 9.1 to fix `__tests__/unit/schema-validator.test.js:341-363` — the `it('should reject quote missing attribution', …)` case (block `{ type:'quote', text:'A quote without attribution' }`, asserts `valid:false`). Post-9.1 that block is VALID. Repurpose it: rename to `'accepts a quote with attribution omitted (F3 crystallization, CR-2)'` and assert `result.valid).toBe(true)`. The sibling `'should accept valid quote content'` (`:245`, attribution present) stays. (Optional: add a `rejects a quote missing text` case to preserve negative coverage — covered by the new `minLength:1`.)
- Net 9.1 files: `lib/schemas/content-bundle.schema.json`, `lib/__tests__/content-bundle-quote-attribution.test.js` (new), **`__tests__/unit/schema-validator.test.js`**.

**Gap B — fold into Task 9.2 (X-5):** removing `thePlayers.pullQuotes` from `outline.schema.json` (which keeps `additionalProperties:false`) cascades into the console outline editor + its shared fixture. Per-phase author scoped 9.2 to 2 files; the real scope is ~5. Add to 9.2:
- **`console/outline-edit-logic.js`** — `initThePlayers` (`:170`): drop the `pullQuotes:` seed line. `buildThePlayersPayload` (`:242-249`): delete the `quotes` map + `setOrDeleteArray(out,'pullQuotes',quotes)`, and add a defensive `delete out.pullQuotes;` (the builder `deepClone`s `originalSection`, which could carry a legacy `pullQuotes`). Leave `validateJournalistOutlineShape` as-is (it never checked `thePlayers` sub-keys — the rejection is purely server-side Ajv, which is what we're aligning to).
- **`console/components/checkpoints/Outline.js`** — `ThePlayersEditor` (`:256-269`): remove the entire `'Pull Quotes (optional)'` `ObjectListEditor` element. Collapsed preview (`:688`): drop the `+ … + ' pull quote(s)'` tail so it reads only `… + ' arc connection(s)'`.
- **`__tests__/unit/outline-edit-logic.test.js`** — fixture `validJournalistOutline().thePlayers` (`:54-56`): remove the `pullQuotes` array (this single edit un-breaks every `validate('outline', full)` test that was failing on the now-stray key, incl. the capstone roundtrip `:629` and the followTheMoney/whatsMissing/closing section tests). Then reconcile the 4 pull-quote-specific tests: `:251` (drop the two `s.pullQuotes…` asserts; keep the `characterHighlights` assert + rename); `:329` (drop the `state.pullQuotes = L.addRow(…)` line + the four `out.pullQuotes[1]…` asserts; keep map/no-stray + `validate(…true)`); `:349` (delete entirely — purely pullQuote-default behavior); `:398` (re-base the "edit one field, rest untouched" trigger onto a surviving field, e.g. `state.exposed = […]` or a `characterHighlights` edit, keep the preservation asserts). KEEP `:592` (stray ROOT `pullQuotes` B4 — unrelated, still valid).
- **`lib/__tests__/outline-no-pullquotes.test.js`** (new, plan body Step 1): correct the `buildOutlinePrompt` call — real signature is `buildOutlinePrompt(arcAnalysis, selectedArcs, heroImage, availablePhotos=[], …)` and BOTH branches call `selectedArcs.map` (`:360` journalist), so the plan's `{exposed,buried}` arg throws. Use: `await builder.buildOutlinePrompt({ narrativeArcs: [{ id:'arc-1', title:'The Money' }] }, ['The Money'], null)`.
- ✅ Verified clean / no change: `__tests__/unit/server-build-resume-payload.test.js` (its `thePlayers` fixture has no pullQuotes; its B4 test targets ROOT pullQuotes); single `content-bundle.schema.json` (no skill-path divergence); both `quote.hbs` partials already guard `{{#if attribution}}` (attribution-less crystallization renders cleanly — no template change).
- Net 9.2 files: `lib/prompt-builder.js`, `lib/schemas/outline.schema.json`, **`console/outline-edit-logic.js`**, **`console/components/checkpoints/Outline.js`**, **`__tests__/unit/outline-edit-logic.test.js`**, `lib/__tests__/outline-no-pullquotes.test.js` (new).

**Task 9.3 (CR-8):** as written, re-anchored to `ai-nodes.js:1192-1212`. The repo-wide sweep confirms `minPullQuotes`/`pullQuoteCount` occur ONLY in that block, so the source-shape guard test passes cleanly after removal. No other consumer.
...
> **NOTE (2026-06-22):** The three `scripts/e2e-walkthrough.js` edits committed as WIP at `4d5a79a` (they were the "dirty" working tree carried through the P1–P4 work) ARE the in-progress starting point of THIS task — Step 6's and Step 7's "BEFORE" blocks are written to match them verbatim (the added `rosterPronouns`/`directorNotes.rawProse` line at `:611` and the band-aid `!currentData` guard at `:3439`). The third edit (roster+pronouns plumbing in `handleAwaitRoster` at `:1981`) is additive incremental-input plumbing kept as-is. So this task BUILDS ON `4d5a79a`: it adds the `sse-complete.js` resolver, finalizes `directorNotes` to the full structured object (Step 6), and replaces the band-aid guard with a fail-loud assertion (Step 7). **Step 5b is the folded #18 harness regression** (P4.4's SSE-1 split made failures emit `type:'failed'`, which the harness must now handle).
