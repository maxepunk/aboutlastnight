...
- **Subagents for context isolation** (image analysis, evidence curation, validation)
- **Single SDK call for arc analysis** (player-focus requires shared context)
...
Phase 2: ARC ANALYSIS → arc_analysis (JSON) → ★ USER SELECTS ARCS
         └── Single SDK call with player-focus-guided three-lens analysis (8.15)
...
| 2 Arc Analysis | Single SDK call | Player-focus requires unified context; three-lens analysis embedded in prompt (8.15) |
...
- `journalist-arc-analyzer` - Director observations drive arc selection (now integrated into single SDK call)
...
// Phase 2: Arc analysis via single SDK call (Commit 8.15)
// Player-focus-guided analysis with three-lens embedding
// See lib/workflow/nodes/arc-specialist-nodes.js for implementation
// No Task tool invocation - uses sdkQuery() directly with embedded prompts
...
**Implementation:** `lib/workflow/nodes/arc-specialist-nodes.js` uses single `sdkQuery()` call with player-focus-guided prompts.
...
### 2.1 Arc Analysis via Single SDK Call (Commit 8.15)

The server workflow uses a single `sdkQuery()` call with embedded three-lens analysis:
...
**Why single-call?** Player focus must guide ALL analysis. Parallel specialists couldn't share:
- The accusation context (who was blamed and why)
- Whiteboard conclusions (what players decided)
- Director observations (what actually happened)

**Three-lens analysis embedded in prompt:**
1. Financial patterns (transaction timing, account naming, value clustering)
2. Behavioral patterns (character dynamics, zero-footprint analysis)
3. Victimization patterns (targeting, operator/victim identification)

**After SDK call returns:**
1. Parse structured arc analysis output
...
The SDK call returns structured JSON written to `analysis/arc-analysis.json` with:
